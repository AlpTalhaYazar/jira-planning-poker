...
export const isSession = (value: unknown): value is Session => {
  ...
  return (
    ...
    (typeof session.currentIssueKey === 'string' || session.currentIssueKey === null) &&
...
export const isProjectConfig = (value: unknown): value is ProjectConfig => {
  ...
  return (
    ...
    (config.estimateFieldId === undefined || typeof config.estimateFieldId === 'string') &&
    (config.deckValues === undefined || isStringArray(config.deckValues)) &&
    (config.defaultJql === undefined || typeof config.defaultJql === 'string') &&
