...
export const assertProjectContext = (req: ForgeProjectContext, requestedProjectKey?: string | null): string => {
  const contextKey = getContextProjectKey(req);
  if (requestedProjectKey && requestedProjectKey !== contextKey) {
    throw new Error('Project mismatch. Please open the app from the intended project.');
  }
  return contextKey;
};
