...
export function ActiveSession({ sessionId }) {
    ...
    const isModerator = participants.find((p) => p.accountId === myAccountId)?.isModerator ?? false;
    if (loading && !session) {
        return _jsx("div", { className: "flex items-center justify-center h-full", children: "Loading session..." });
    }
    if (error) {
        return _jsxs("div", { className: "flex items-center justify-center h-full text-red-500", children: ["Error: ", error] });
    }
    if (!session) {
        return _jsx("div", { className: "flex items-center justify-center h-full", children: "Session not found" });
    }
    // Handlers
    const handleVote = async (value) => {
        if (!currentIssue || currentIssue.isRevealed)
            return;
    ...
    const handleSubmitEstimate = async (value) => {
        if (!currentIssue)
            return;
        try {
            setIsActionLoading(true);
    ...
    const handleSearch = async (jql) => {
        if (!session)
            return;
        try {
            setIsActionLoading(true);
            // First fetch issues to get keys
            const issues = await api.getIssuesForProject(session.projectKey, { jql, maxResults: 50 });
    ...
    if (currentIssue?.votes) {
        Object.entries(currentIssue.votes).forEach(([uid, vote]) => {
    ...
    return (_jsxs("div", { className: "flex flex-col h-[calc(100vh-140px)] min-h-[600px] bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden", children: [_jsxs("div", { className: "border-b border-slate-200 p-4 bg-slate-50/50 flex justify-between items-center", children: [_jsx(JQLBar, { onSearch: handleSearch, loading: isActionLoading, initialValue: session.jql }), _jsx(SessionControls, { session: session, isModerator: isModerator, onPause: handlePause, onResume: handleResume, onComplete: handleComplete, onSettings: handleSettings, loading: isActionLoading })] }), _jsx(SessionSettingsDialog, { open: isSettingsOpen, onOpenChange: setIsSettingsOpen, session: session, onUpdate: () => {
                    // Refetch session if needed
                    // refetch();
                } }), session.status === 'paused' && (_jsx("div", { className: "bg-yellow-50 p-2 text-center text-yellow-800 text-sm font-medium border-b border-yellow-100", children: "Session is paused" })), _jsxs(ResizablePanelGroup, { direction: "horizontal", children: [_jsx(ResizablePanel, { defaultSize: 30, minSize: 20, maxSize: 40, children: _jsx(IssuePanel, { issue: displayIssue, total: session.issueKeys.length, current: session.issueKeys.indexOf(session.currentIssueKey || '') + 1, onNext: handleNext, onPrev: handlePrev }) }), _jsx(ResizableHandle, { withHandle: true }), _jsx(ResizablePanel, { defaultSize: 70, children: _jsxs("div", { className: "flex flex-col h-full bg-slate-50/30 relative", children: [_jsx("div", { className: "flex-1 p-8 flex items-center justify-center", children: _jsx(VotingTable, { votes: votesForTable, isRevealed: currentIssue?.isRevealed || false, onReveal: handleReveal, finalEstimate: null, onSubmitEstimate: handleSubmitEstimate, participants: participants.map((p) => ({
                                            id: p.accountId,
                                            accountId: p.accountId,
                                            name: p.displayName || p.name || 'Unknown',
                                            avatar: p.avatarUrl || '',
                                            connectionStatus: p.connectionStatus
                                        })), isModerator: isModerator }) }), _jsx("div", { className: "p-6 bg-white border-t border-slate-200 z-10", children: _jsx(VotingDeck, { options: session.deckValues, selectedValue: myVote, onSelect: handleVote, onRetract: handleRetract, disabled: currentIssue?.isRevealed || session.status !== 'active' }) })] }) })] })] }));
}
