...
const createSocketMock = () => {
  ...
  const anyListeners: Array<(event: string, payload: any) => void> = [];
  const socket = {
    on: vi.fn((event: string, callback: SocketListener) => {
      const existing = listeners.get(event) ?? [];
      listeners.set(event, [...existing, callback]);
      return socket;
    }),
    onAny: vi.fn((callback: (event: string, payload: any) => void) => {
...
describe('useRealtimeSession', () => {
  ...
  it('emits each session event only once', async () => {
    ...
    act(() => {
      socketMock.emit('session:event', { sessionId: 'SESSION-1', event: 'session:event' });
    });

    expect(onSessionEvent).toHaveBeenCalledTimes(1);
    expect(onSessionEvent).toHaveBeenCalledWith({ sessionId: 'SESSION-1', event: 'session:event' });
