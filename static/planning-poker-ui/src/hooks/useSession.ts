...
const normalizeSessionResponse = (data: any): SessionData | null => {
...
export function useSession(sessionId: string | null) {
  ...
  const connectRealtime = useCallback(async () => {
    ...
    try {
      ...
      const tokenData = await api.getRealtimeToken(sessionId) as {
        token: string | null;
        ...
        setSession(prev => {
          ...
          return {
            ...
            participants: prev.participants.map(p =>
              p.accountId === payload.participantId
      ...
      realtimeClient.on('issue.revealed', () => {
        setSession(prev => {
          if (!prev || !prev.currentIssue) return prev;
          return {
            ...prev,
