...
interface UseRealtimeSessionProps {
    sessionId: string;
    onSessionEvent: (message: { sessionId?: string; event?: string; payload?: unknown }) => void;
    onSocketEvent?: (eventName: string, payload: unknown) => void;
    onToken?: (tokenResponse: { token: string; relayUrl: string; expiresAt: string | null }) => void;
}

export function useRealtimeSession({ sessionId, onSessionEvent, onSocketEvent, onToken }: UseRealtimeSessionProps) {
    ...
    useEffect(() => {
        ...
        const connect = async () => {
            ...
            try {
                ...
                onToken?.(tokenResponse as { token: string; relayUrl: string; expiresAt: string | null });
                if (cancelled) {
                    return;
                }
