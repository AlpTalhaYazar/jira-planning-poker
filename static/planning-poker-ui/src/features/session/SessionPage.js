...
import { applyEstimate as applyEstimateRequest, castVote as castVoteRequest, clearVotes as clearVotesRequest, fetchIssuesForProject, getSession as fetchSessionDetails, leaveSession as leaveSessionApi, revealIssue as revealIssueRequest, setCurrentIssue as setCurrentIssueRequest, updateSessionBacklog as updateSessionBacklogRequest, } from "../../api/sessionsClient";
...
const defaultJqlForSession = (session) => session.jql ??
    (session.projectKey
        ? `project = "${session.projectKey}" ORDER BY updated DESC`
        : "");
export default function SessionPage({ data, onBack, onSessionData, viewerAccountId, projectConfig, onDebugEvent, }) {
    ...
    const currentIssueState = currentIssue && data.currentIssueState?.issueKey === currentIssue.key
    ...
    const logDebug = useCallback((direction, eventName, payload) => {
        const sanitizedPayload = eventName === "token" && payload && typeof payload === "object"
            ? {
    ...
    const logOutgoing = useCallback((eventName, payload) => {
        logDebug("outgoing", eventName, payload);
    }, [logDebug]);
    const captureSessionEvent = useCallback((message) => {
    ...
    const localParticipantId = (viewerAccountId &&
    ...
    const handleCardSelect = (value) => {
        ...
        void runAction(() => castVoteRequest(session.id, currentIssue.key, value), "Unable to submit your vote. Please try again.", false).then((success) => {
            if (!success) {
                setPendingVotes((prev) => {
                    const next = { ...prev };
    ...
    return (_jsxs("div", { className: "session-layout", children: [_jsxs("div", { className: "session-top-bar", children: [_jsx("button", { className: "text-button", type: "button", onClick: onBack, children: "\u2190 Sessions" }), _jsx("span", { className: "session-name", children: session.name }), realtimeStatus !== "connected" && (_jsxs(_Fragment, { children: [_jsx("span", { className: "meta-text", role: "status", children: realtimeStatus === "disabled"
                                    ...
                                    : null, onSelect: handleCardSelect, disabled: isRevealed || isSubmittingAction, isRevealed: isRevealed }), isRevealed && (_jsxs("section", { className: "results-panel", children: [_jsxs("header", { className: "panel-heading", children: [_jsx("h3", { children: "Vote results" }), _jsx("span", { className: "meta-text", children: "Visible to everyone" })] }), _jsxs("dl", { className: "results-grid", children: [_jsxs("div", { children: [_jsx("dt", { children: "Average" }), _jsx("dd", { children: stats.average })] }), _jsxs("div", { children: [_jsx("dt", { children: "Median" }), _jsx("dd", { children: stats.median })] }), _jsxs("div", { children: [_jsx("dt", { children: "Min" }), _jsx("dd", { children: stats.min })] }), _jsxs("div", { children: [_jsx("dt", { children: "Max" }), _jsx("dd", { children: stats.max })] })] }), viewerIsModerator && (_jsxs("div", { className: "apply-estimate", children: [_jsxs("header", { children: [_jsx("h4", { children: "Apply estimate to Jira" }), !projectConfig?.estimateFieldId && (_jsx("p", { className: "error-text", children: "Configure an estimate field before applying." }))] }), _jsx("div", { className: "apply-estimate__chips", children: (projectConfig?.deckValues ?? session.deckValues).map((value) => (_jsx("button", { type: "button", className: `chip ${applyValue === value ? "chip--selected" : ""}`, onClick: () => {
                                                ...
                                                }, "Unable to apply estimate.", true), disabled: isSubmittingAction ||
                                                    !applyValue ||
                                                    !projectConfig?.estimateFieldId, children: "Apply to Jira" }), applyMessage && (_jsx("p", { className: "success-text", children: applyMessage }))] }))] }))] })] })) : (_jsx("section", { className: "empty-panel", children: _jsx("p", { children: isFetchingIssues
                        ? "Loading issues…"
